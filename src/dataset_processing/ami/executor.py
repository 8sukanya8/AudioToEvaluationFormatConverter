...
input_directory = "/Users/sukanyanath/Documents/PhD/Internship_Spinning_Bytes/Datasets/Diarization/AMI/ami_public_manual_1/segments"
output_directory = "/Users/sukanyanath/Documents/PhD/Internship_Spinning_Bytes/Datasets/Diarization/AMI/ami_public_manual_1/rttm"

def execute_ami_dataset_to_rttm_file(input_directory, output_directory):
    """
...
def convert_audio_file_segments_to_rttm_file(file_segments_dict, input_directory, output_directory):
    ...
    for audio_file_name in file_segments_dict.keys():
        ...
        for segment in author_segments:
            author_name = segment.split(".")[1]
            rttm_list = rttm_list + read_xml_into_rttm_list(audio_file_name, input_directory+"/"+segment, author_name)
...
def read_xml_into_rttm_list(audio_file_name, xml_file_path, author_name):
    """
    ...
    for child in root.getchildren():
        if len(child.attrib) > 0 :
            ...
            time_start = float(child.attrib['transcriber_start'])*100
            duration = (float(child.attrib['transcriber_end']) - float(child.attrib['transcriber_start']))*100
